...
export default function CatalogDrawer({
  ...
  // Group the filtered items by category and render a lightweight sticky header at each category boundary when 'All Items' is selected
  const groupedFilteredItems = useMemo(() => {
    if (selectedCategory !== "All Items") {
      return [{ category: selectedCategory, items: filteredItems }];
    }
    const map: Record<string, RegularItem[]> = {};
    ...
    return sortedCats.map((cat) => ({ category: cat, items: map[cat] }));
            ...
            <div className="pb-8">
              {groupedFilteredItems.map(({ category, items }) => (
                <div key={category} className="flex flex-col">
                  ...
                  <div className="sticky top-0 z-10 bg-surface border-b border-outline/10 px-4 py-1.5 flex justify-between items-center select-none">
                    <span className="text-[11px] font-black uppercase tracking-wider text-on-surface-variant">
                      {category}
                    </span>
                    <span className="text-[10px] bg-primary/10 text-primary px-2 py-0.5 rounded-full font-extrabold font-tnum">
                      {items.length} {items.length === 1 ? "Item" : "Items"}
                    </span>
                  </div>

                  {/* High-density Flat Row List */}
                    ...
                    {items.map((item) => {
                      ...
                      return (
                        <div
                          ...
                          className={`cursor-pointer select-none transition-all duration-200 py-2.5 px-4 flex flex-col gap-1 hover:bg-surface-container-low active:scale-[0.99] ${
                            isAdded
                              ? "bg-primary/[0.04]"
                              : "bg-surface"
                          }`}
                        >
                          <div className="flex items-center justify-between gap-4 w-full">
                            <div className="flex flex-col flex-grow min-w-0">
                              ...
                              {cheapestPriceStr && (
                                <div className="flex items-center gap-2 flex-wrap font-tnum mt-0.5">
                            ...
                            <div className="flex flex-wrap gap-1 mt-1.5">
                              {storesWithPrices.map((sp) => (
                                <button
                                  ...
                                  className={`px-2 py-0.5 rounded-full text-[10px] font-extrabold border cursor-pointer select-none transition-all hover:scale-105 active:scale-95 flex items-center gap-1 ${getStoreBadgeClass(sp.storeId)}`}
                                >
                                  {sp.onSale && <Flame size={10} className="fill-amber-600 stroke-none" />}
                          ...
                          {expandedItemPrices.has(item.id) && storesWithPrices.length > 0 && (
                            <div 
                              ...
                              className="mt-2 pt-2 border-t border-outline/10 space-y-1.5 cursor-default w-full"
                            >
                              <div className="text-[9px] font-black text-on-surface-variant uppercase tracking-wider mb-1">
