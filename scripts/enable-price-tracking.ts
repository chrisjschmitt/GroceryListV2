...
async function migrate() {
  ...
  // 2. Update canonical Combined Catalog in MongoDB
  console.log("\n--- Part 1: Updating MongoDB Catalog ---");
  let catalog;
  try {
    catalog = await blobGetCombinedCatalog();
  } catch (err: any) {
    console.error("Error fetching catalog from MongoDB:", err.message || err);
    process.exit(1);
  }
  ...
  if (catalogUpdatesCount > 0) {
    try {
      ...
      console.log("Successfully saved updated catalog back to MongoDB!");
    } catch (err: any) {
      console.error("Failed to save updated catalog to MongoDB:", err.message || err);
      process.exit(1);
    }
  } else {
    console.log("No catalog updates required (all verified non-Costco links are already tracked).");
  }
  ...
  try {
    ...
    for (const doc of mongoDocs) {
      ...
      else if (lowerStoreId.includes("canadiantire")) storeKey = "canadiantireperth";
